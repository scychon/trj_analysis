...
!  --------------------------------------------------------------------------
  ...
  real*8             :: volavg, muavg
  real*8,dimension(3)    :: xpos,xdrudepos,compos,drudepairpos,xposrel, drudepairposrel   ! temporary position vector for x, center of mass(com)
  real*8, allocatable,dimension(:,:) :: comtraj, comtrajmol   ! trajectory of com of each molecules (frame idx, mol idx, xyz)
  real*8, allocatable,dimension(:,:) :: atomveltraj, atomreltraj   ! trajectory of com of each molecules (frame idx, mol idx, xyz)
  ...
  real*8, allocatable,dimension(:) :: keframe,kerelframe     !matrix for kinetic energy of each atom in the frame
...
! ----------------------------------------------------------------
  ...
  if(narg>0)then
    ...
    ! initialize all matrices for first trajectory 
      ...
      allocate(atomreltraj(nsize,natomtypes))
    ...
    do while ( trajin % STAT == 0 )
        ...
        if(isize .lt. ixtc) then
          ...
          call expand2D(atomreltraj,nsize,0)
        ...
        do i=1, sys % nummoltype
          ...
          !$OMP   SHARED (comtraj,keframe,kerelframe)
          !$OMP DO
          do j=1, nmolcurr
            ...
            !!! calculate the atom kinetic energy & relative kinetic energy
            idxdrude = 0
            do k=1, natomcurr
              idx = idxatom + (j-1) * natomcurr + k
              mass = sys % moltype(i) % atommass(k)
              ...
              if (drudepairidx .gt. k) then
                ...
                drudepairposrel(:) = drudepairpos(:) - (mass + drudemass) * compos(:) / molmass
                kerelframe(idx) = dot_product(drudepairposrel,drudepairposrel)/(mass+drudemass)
                ...
                kerelframe(idx) = keframe(idx)
              else
                keframe(idx) = mass * dot_product(xpos,xpos)
                xposrel = xpos - compos/molmass
                kerelframe(idx) = mass * dot_product(xposrel,xposrel)
              endif
            enddo
          ...
          !!$OMP PARALLEL &
          !!$OMP   DEFAULT (FIRSTPRIVATE) &
          !!$OMP   SHARED (atomveltraj,atomreltraj)
          !!$OMP DO
          do j=1, sys % moltype(i) % numatom
            ...
            tempval = sum(kerelframe(idxstart:idxend:idxskip))/float(nmolcurr)
            atomreltraj(ixtc,idxatomtype+j) = tempval
 ...
 100    FORMAT(A, I8,A)  
    ...
    if(isize .gt. nxtc) then
      ...
      call shrink2D(atomreltraj,nxtc,natomtypes)
    ...
    write(strfmt,'("( F12.3, ",I0,"ES15.7 )")') nmoltype + natomtypes*2
    do idt=1,ndframe
      write(18,strfmt) dt0*idt,comtrajmol(idt,:)*mult,atomveltraj(idt,:)*mult,atomreltraj(idt,:)*mult
    enddo
