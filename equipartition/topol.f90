...
contains
    ...
    subroutine init_top(top,filename_in)
        ...
        integer :: drudepairidx, ndrudepair
        real*8  :: mass, charge
        ...
        do i=1, nmoltype
          ...
          allocate(top % moltype(i) % atomdrudepairidx(natom))

          ndrudepair = 0
          do j=1, natom
            read(7,*) atomname, mass, charge, drudepairidx
            top % moltype(i) % atommass(j) = mass
            top % moltype(i) % atomcharge(j) = charge
            top % moltype(i) % atomdrudepairidx(j) = drudepairidx
            top % moltype(i) % molarmass = top % moltype(i) % molarmass + mass
            top % moltype(i) % molcharge = top % moltype(i) % molcharge + charge
            if (drudepairidx .gt. 0) then
                ndrudepair = ndrudepair + 1
            endif
          enddo
          top % moltype(i) % numdrudepair = ndrudepair
        enddo
