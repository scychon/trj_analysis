...
!  --------------------------------------------------------------------------
  ...
  integer            :: nvar =2                     !# of output variables
  ...
  integer            :: nPUMP=0,nCHEM=0                 !resulting total number of PUMP & CHEM conf
  integer            :: nPUMPframe=0,nCHEMframe=0                 !resulting total number of PUMP & CHEM conf
  ...
  integer,allocatable,dimension(:)    :: nPUMPxtc,nCHEMxtc,nframextc
  ...
  real*8             :: dist,rPUMPxtc,rCHEMxtc,pi
...
if(.not.fileExist)then
  ...
  stop
    ...
    allocate(nPUMPxtc(nxtcfile))
    allocate(nCHEMxtc(nxtcfile))
...
!        write(*,*) 'found hbonding network'
        ...
        if(bPUMP(1)) then
          hbondcnt(1,ixtc)= 1
          nPUMP = nPUMP+1
          nPUMPframe = nPUMPframe+1
          nPUMPxtc(ifile) = nPUMPxtc(ifile)+1
        endif
        if(bCHEM(1)) then
          hbondcnt(2,ixtc)= 1
          nCHEM = nCHEM+1
          nCHEMframe = nCHEMframe+1
          nCHEMxtc(ifile) = nCHEMxtc(ifile)+1
        endif



        write(6,100) ixtc,'th frame has finished  ' , hbondcnt(1,ixtc)
 100    FORMAT('+', I7,A,I)  

        call traj % read
        write(16, '(f12.6,I,I)') traj % time, hbondcnt(:,ixtc)
        if(mod(ixtc,nframe).eq.0) then
          write(17, '(f12.6,I,f12.6,I,f12.6)') traj % time, nPUMPframe, real(nPUMPframe)/real(nframe),nCHEMframe,real(nCHEMframe)/real(nframe)
          nPUMPframe = 0
          nCHEMframe = 0
        endif
    ...
    do ifile=1,nxtcfile
      ...
      write(16,'(A,f12.6,A,f12.6,f12.6)') 'number of PUMP configurations :', rPUMPxtc, ' number of CHEM configurations :', rCHEMxtc
    enddo
