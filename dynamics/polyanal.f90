...
!  --------------------------------------------------------------------------
  ...
  integer            :: i,j,k,l,icnt,idx,idxl,ixtc,idxmol,idxatom,idxmoltype           !counters
  ...
  real*8,dimension(3)    :: xpos,xposl,compos,composl   ! temporary position vector for x, center of mass(com)
...
!        write(*,*) 'volume', volume, trajin % box(1,1), trajin % box(2,2), trajin % box(3,3)
        ...
        do i=1, sys % nummoltype
          ...
          !$OMP   SHARED (unitNormMolTraj, comtraj, mutraj, polyRgSq, polyReteSq)
          !$OMP DO
