...
!  --------------------------------------------------------------------------
  ...
  real*8, allocatable,dimension(:,:,:) :: comtraj,comtrajcat,comtrajani,mutraj,mutrajcat,mutrajani,temp   ! trajectory of com of each molecules (xyz, mol idx, frame idx)
  real*8, allocatable,dimension(:,:) :: comMolt1,comMolt2,comMolDiff   ! array for com of each molecules at specific time (xyz, mol idx)
  ...
  real*8, allocatable,dimension(:) :: invnmolcurr    ! array for number of molecules in each moltype ( moltype idx )
  ...
  integer,allocatable,dimension(:,:)   :: idtmat   !  (jxtc, ixtc)

  ! for conductivity calculation
  ! matrices
  real*8, allocatable,dimension(:,:,:) :: xqcomTraj     ! matrix for q*xyz_com of each molecule at each timestep (q*xyz, mol idx, frame idx)
  real*8, allocatable,dimension(:,:,:) :: xqAtomsTraj     ! matrix for q*xyz of each atom at each timestep (q*xyzx, atom idx, frame idx)
  ...
  real*8, allocatable,dimension(:,:) :: xqcomDiff     ! matrix for difference of xq_xyz of each molecules at each time step combination (xyz,idxmol)
  real*8,dimension(3)    :: xqdiff1,xqdiff2,xqdiff


  ! for MSD calculation
  real*8                    :: msd
  real*8,dimension(3)       :: xdiff
  real*8, allocatable,dimension(:,:)  :: msdTime      ! translational mean square displacement of each molecule types at each time difference (moltype idx i, time difference dt in unit of time step
  ...
  real*8, allocatable,dimension(:,:) :: unitNormMolt1, unitNormMolt2     ! unit normal vector of each molecules at each timestep (vec_xyz, mol idx, frame idx)
  ...
  real*8, allocatable,dimension(:,:) :: rotACFTime,rotACFTimeP2     ! rotational ACF <ui(t2).ui(t1)> of each molecule types at each time difference (moltype idx i, time difference dt in unit of time step )
  real*8, allocatable,dimension(:) :: rotACFt0     ! rotational ACF <ui(t2).ui(t1)> of each molecule types at each time difference (moltype idx, time difference dt in unit of time step )
...
! ----------------------------------------------------------------
  ...
  if(narg>0)then
    ...
    invnmolsys = 1.d0/nmolsys
    allocate(molTypeIdxs(nmolsys))
    allocate(invnmolcurr(nmolsys))
    idxmol = 0
    do i=1, sys % nummoltype
      ...
      do j=1, nmolcurr
        ...
        invnmolcurr(idxmol) = 1.d0/(nmolcurr)
      enddo
    enddo
    ...
    if(nsysatoms .eq. 0) then
      ...
      allocate(comtraj(3,nmolsys,nsize))
      allocate(mutraj(3,nmolsys,nsize))
      allocate(unitNormMolTraj(3,nmolsys,nsize))
!      allocate(xqAtomsTraj(nsize,nsysatoms,3))
      allocate(boxtraj(3,nsize))
...
!        write(*,*) 'volume', volume, traj % box(1,1), traj % box(2,2), traj % box(3,3)
        ...
        if(isize .lt. ixtc) then
          ...
          call expand3D(comtraj,0,0,nsize)
          call expand3D(mutraj,0,0,nsize)
          call expand3D(unitNormMolTraj,0,0,nsize)
        !  call expand3D(xqAtomsTraj,nsize,0,0)
          call expand2D(boxtraj,0,nsize)
        ...
        do i=1, sys % nummoltype
          do j=1, sys % moltype(i) % nummol
            ...
            enddo
            unitNormMolTraj(:,idxmol,ixtc) = vecUnitNorm( xPlaneAtom(:,1)-xPlaneAtom(:,2), xPlaneAtom(:,1)-xPlaneAtom(:,3) )

            molmass = sys % moltype(i) % molarmass
            compos(:) = compos(:)/molmass
            comtraj(:,idxmol,ixtc) = compos(:)
            mutraj(:,idxmol,ixtc) = mutot(:)
!            write(*,*) comtraj(:,idxmol,ixtc)
          enddo
        enddo 
 ...
 100    FORMAT('+', I5,A)  
    ...
    !  call shrink3D(xqAtomsTraj,3,nsysatoms,nxtc)
...
!    allocate(idtmat(nxtc-1,nxtc))
    ...
    allocate(comMolt1(3,nmolsys))
    !allocate(comMolt2(3,nmolsys))
...
!    allocate(xqAtomsCFTime(ndframe))
    ...
    allocate(msdTime(nmoltype+1,ndframe))
    write(6,*) 'msdTime allocated' 
    allocate(rotACFTime(nmoltype+1,ndframe))
    write(6,*) 'rotACFTime allocated' 
    allocate(rotACFTimeP2(nmoltype+1,ndframe))
    ...
    do i=1,nxtc-1,nskip
     ...
     !   nACFTime(idt) = nACFTime(idt) + 1
        ...
        do k=1,nmolsys
          ...
          invnmol = invnmolcurr(k)
          xdiff(:) = comMolDiff(:,k)
          msd = dot_product(xdiff, xdiff)
          msdTime(idxmoltype,idt) = msdTime(idxmoltype,idt) + msd*invnmol
          msdTime(nmoltype+1,idt) = msdTime(nmoltype+1,idt) + msd*invnmolsys
          
          rotacf = dot_product(unitNormMolt1(:,k),unitNormMolt2(:,k))
          rotACFTime(idxmoltype,idt) = rotACFTime(idxmoltype,idt) + rotacf*invnmol
          rotACFTime(nmoltype+1,idt) = rotACFTime(nmoltype+1,idt) + rotacf*invnmolsys
          rotacf = rotacf*rotacf
          rotACFTimeP2(idxmoltype,idt) = rotACFTimeP2(idxmoltype,idt) + rotacf*invnmol
          rotACFTimeP2(nmoltype+1,idt) = rotACFTimeP2(nmoltype+1,idt) + rotacf*invnmolsys

          xqdiff(:) = xqdiff(:) + xqcomDiff(:,k)
        enddo

       ! do k=1,nmolsys
       !   do l=1,nmolsys
       !     xqdiff2(:) = xqcomDiff(:,l)
       !     xqcomCFTime(idt) = xqcomCFTime(idt) + dot_product(xqdiff1,xqdiff2)
       !   enddo
       ! enddo
        xqcomCFTime(idt) = xqcomCFTime(idt) + dot_product(xqdiff,xqdiff)
...
!      endif
      if(msdTime(nmoltype+1,idt) .ne. 0) then
      ...
      if(rotACFTime(nmoltype+1,idt) .ne. 0) then
