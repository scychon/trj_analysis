...
subroutine high_order_drude_energy (i_data,energy)
  ...
  interface
    ...
    ! global variable array "exponents" will be used in electrostatic screening functions.  For drude oscillator screening, the easiest thing to do is to duplicate these exponents for the drude oscillators
                ...
                if ( i_mole /= j_mole ) then  ! if it is not self interaction
                      do i_atom = 1, n_atom( i_mole )
                         do j_atom = 1, n_atom( j_mole )
                            ...
                            f_ij = chg(i_mole,i_atom) * chg(j_mole,j_atom) * rij * screen(i_atom,j_atom,i_mole,j_mole,norm_dr) / norm_dr**3
                            f_ij = f_ij - chg(i_mole,i_atom) * chg(j_mole,j_atom) * dscreen(i_atom,j_atom,i_mole,j_mole,rij) / norm_dr
                ...
                if ( i_mole /= j_mole ) then  ! if it is not self interaction
                   do i_atom = 1, n_atom(i_mole)
                      do j_atom = 1, n_atom(j_mole)
                         ...
                         E_elec_cutoff = E_elec_cutoff + chg(i_mole,i_atom)*chg(j_mole,j_atom)* screen(i_atom,j_atom,i_mole,j_mole,norm_dr) / norm_dr
                      end do
                   end do
...
!!!!!!!!!!!!!!! add screened atom-atom, get atom chg from corresponding oscillator
          ...
          E_intra = dble(sign_chg_i*sign_chg_j)*chg(i_mole,i_drude) * chg(i_mole,j_drude) * screen(i_fix,j_fix,i_mole,i_mole,norm_dr)/ norm_dr
...
!!!!!!!!!!!!add screened atom-drude, get atom chg from corresponding oscillator
          pol1=chg(i_mole,i_drude)**2/springcon; pol2=chg(i_mole,j_drude)**2/springcon
          f_ij= dble(sign_chg_i*sign_chg_j)*chg(i_mole,i_drude) * chg(i_mole,j_drude) * (rij * screen(i_fix,j_fix,i_mole,i_mole,norm_dr)/ norm_dr**3 - dscreen(i_fix,j_fix,i_mole,i_mole,rij) / norm_dr)
       else
          f_ij =0D0
       endif
 ...
 !*****************************************************
  recursive subroutine findpositionshell(n,mon)
    ...
    if(mon.eq.1) then
       ...
       do i=1,atoms1
          x=nxyzshell1(n,i,1)-xyzmon1(n,i,1)
          y=nxyzshell1(n,i,2)-xyzmon1(n,i,2)
          z=nxyzshell1(n,i,3)-xyzmon1(n,i,3)
       ...
       do i=1,atoms2
          x=nxyzshell2(n,i,1)-xyzmon2(n,i,1)
          y=nxyzshell2(n,i,2)-xyzmon2(n,i,2)
          z=nxyzshell2(n,i,3)-xyzmon2(n,i,3)
          dist(1)=x;dist(2)=y;dist(3)=z
          r=(x**2+y**2+z**2)**.5
...
function field1(shell,monomer,n)
  ...
  real*8::dist,sum1,sum2,sum3,x,y,z
...
if(monomer .eq. 1) then
  sum1=0.0
  sum2=0.0
  sum3=0.0
  do i=1,atoms2
!!!!!!!!!!!!!!!field at shell due to atoms on other monomer
     x=(nxyzshell1(n,shell,1)-xyzmon2(n,i,1))
     y=(nxyzshell1(n,shell,2)-xyzmon2(n,i,2))
     z=(nxyzshell1(n,shell,3)-xyzmon2(n,i,3))
     xyz(1)=x;xyz(2)=y;xyz(3)=z;
     dist=((x)**2+(y)**2+(z)**2)**.5
     sum1=sum1+screen(shell,i,1,2,dist)*(chargeatoms2(i))*x/(dist**3)
     sum2=sum2+screen(shell,i,1,2,dist)*(chargeatoms2(i))*y/(dist**3)
     sum3=sum3+screen(shell,i,1,2,dist)*(chargeatoms2(i))*z/(dist**3)
     summ(:)=(-1.)*dscreen(shell,i,1,2,xyz)*(chargeatoms2(i))/dist
     sum1=sum1+summ(1);sum2=sum2+summ(2);sum3=sum3+summ(3)
  enddo

! ********* field due to intramolecular drude oscillator contributions
  do i=1,atoms1
     if(i .ne.shell) then
        x=(nxyzshell1(n,shell,1)-nxyzshell1(n,i,1))
        y=(nxyzshell1(n,shell,2)-nxyzshell1(n,i,2))
        z=(nxyzshell1(n,shell,3)-nxyzshell1(n,i,3))
        xyz(1)=x;xyz(2)=y;xyz(3)=z;
        dist=((x)**2+(y)**2+(z)**2)**.5
        sum1=sum1+screen(shell,i,1,1,dist)*(shellcharge1(i))*x/(dist**3)
        sum2=sum2+screen(shell,i,1,1,dist)*(shellcharge1(i))*y/(dist**3)
        sum3=sum3+screen(shell,i,1,1,dist)*(shellcharge1(i))*z/(dist**3)
        summ(:)=(-1)*dscreen(shell,i,1,1,xyz)*(shellcharge1(i))/dist
        sum1=sum1+summ(1);sum2=sum2+summ(2);sum3=sum3+summ(3)

        x=(nxyzshell1(n,shell,1)-xyzmon1(n,i,1))
        y=(nxyzshell1(n,shell,2)-xyzmon1(n,i,2))
        z=(nxyzshell1(n,shell,3)-xyzmon1(n,i,3))
        xyz(1)=x;xyz(2)=y;xyz(3)=z;
        dist=((x)**2+(y)**2+(z)**2)**.5
        sum1=sum1+screen(shell,i,1,1,dist)*(-shellcharge1(i))*x/(dist**3)
        sum2=sum2+screen(shell,i,1,1,dist)*(-shellcharge1(i))*y/(dist**3)
  ...
  do i=1,atoms1
!!!!!!!!!!!!!!!field at shell due to atoms on other monomer
     x=(nxyzshell2(n,shell,1)-xyzmon1(n,i,1))
     y=(nxyzshell2(n,shell,2)-xyzmon1(n,i,2))
     z=(nxyzshell2(n,shell,3)-xyzmon1(n,i,3))
     xyz(1)=x;xyz(2)=y;xyz(3)=z;
     dist=((x)**2+(y)**2+(z)**2)**.5
     sum1=sum1+screen(shell,i,1,2,dist)*(chargeatoms1(i))*x/(dist**3)
     sum2=sum2+screen(shell,i,1,2,dist)*(chargeatoms1(i))*y/(dist**3)
     sum3=sum3+screen(shell,i,1,2,dist)*(chargeatoms1(i))*z/(dist**3)
     summ(:)=(-1.)*dscreen(shell,i,1,2,xyz)*(chargeatoms1(i))/dist
     sum1=sum1+summ(1);sum2=sum2+summ(2);sum3=sum3+summ(3)
  enddo
...
! ********* field due to intramolecular drude oscillator contributions
  do i=1,atoms2
     if(i .ne.shell) then
        x=(nxyzshell2(n,shell,1)-nxyzshell2(n,i,1))
        y=(nxyzshell2(n,shell,2)-nxyzshell2(n,i,2))
        z=(nxyzshell2(n,shell,3)-nxyzshell2(n,i,3))
        xyz(1)=x;xyz(2)=y;xyz(3)=z;
        dist=((x)**2+(y)**2+(z)**2)**.5
        sum1=sum1+screen(shell,i,2,2,dist)*(shellcharge2(i))*x/(dist**3)
        sum2=sum2+screen(shell,i,2,2,dist)*(shellcharge2(i))*y/(dist**3)
        sum3=sum3+screen(shell,i,2,2,dist)*(shellcharge2(i))*z/(dist**3)
        summ(:)=(-1)*dscreen(shell,i,2,2,xyz)*(shellcharge2(i))/dist
        sum1=sum1+summ(1);sum2=sum2+summ(2);sum3=sum3+summ(3)

        x=(nxyzshell2(n,shell,1)-xyzmon2(n,i,1))
        y=(nxyzshell2(n,shell,2)-xyzmon2(n,i,2))
        z=(nxyzshell2(n,shell,3)-xyzmon2(n,i,3))
        xyz(1)=x;xyz(2)=y;xyz(3)=z;
        dist=((x)**2+(y)**2+(z)**2)**.5
        sum1=sum1+screen(shell,i,2,2,dist)*(-shellcharge2(i))*x/(dist**3)
        sum2=sum2+screen(shell,i,2,2,dist)*(-shellcharge2(i))*y/(dist**3)
