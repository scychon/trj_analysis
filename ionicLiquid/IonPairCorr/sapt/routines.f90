...
CONTAINS
  ...
  !*****************************************************
  SUBROUTINE getdata(inpfile,datpts,atoms1,atoms2,E1pol,E1exch,E2ind,E2indexch,E2disp,E2dispexch,E1tot,E2tot,Etot,E2ind12,E2ind21,dhf,xyzmon1,xyzmon2,atomtypemon1,atomtypemon2)
    IMPLICIT NONE
    CHARACTER(*),INTENT(in)::inpfile
...
!!!!!!!!!!!!!!!!!!!!!!! read drude charges and springcon
  ...
  subroutine writeoutput(out_file,rms)
    ...
    Case("yes")
       write(7,*) "We have included higher order drude oscillator contributions in the dhf fit.  These contributions are energy differences between fully self-consistent optimized drude positions and drude responses to only the static charges"
    End Select
  ...
  subroutine initialize_data(data_file, datpts,atoms1,atoms2)
    ...
    !  allocate everything
       ALLOCATE(atomtypemon1(atoms1),atomtypemon2(atoms2),chargeatoms1(atoms1),chargeatoms2(atoms2),shellcharge1(atoms1),shellcharge2(atoms2),exponents(atoms1,atoms2),Exoverlap(atoms1,atoms2),Elecoverlap(atoms1,atoms2),Inducoverlap(atoms1,atoms2),dhfoverlap(atoms1,atoms2),Elecatom1(atoms1),Elecatom2(atoms2),Inducatom1(atoms1),Inducatom2(atoms2),Exchatom1(atoms1),Exchatom2(atoms2),dhf1(atoms1),dhf2(atoms2),Cn_cross(atoms1,atoms2,4))

    ALLOCATE(xyzmon1(datpts,atoms1,3),xyzmon2(datpts,atoms2,3), E1pol(datpts),E1exch(datpts),E2ind(datpts),E2indexch(datpts),E2disp(datpts),E2dispexch(datpts),Ecomp(6,datpts),Eout(6,datpts),E1tot(datpts),E2tot(datpts),Etot(datpts),E2ind12(datpts),E2ind21(datpts),dhf(datpts),nxyzshell1(datpts,atoms1,3),nxyzshell2(datpts,atoms2,3),Etot_noind(datpts))

    CALL getdata(data_file,datpts,atoms1,atoms2,E1pol,E1exch,E2ind,E2indexch,E2disp,E2dispexch,E1tot,E2tot,Etot,E2ind12,E2ind21,dhf,xyzmon1,xyzmon2,atomtypemon1,atomtypemon2)
