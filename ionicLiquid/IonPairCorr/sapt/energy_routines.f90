...
FUNCTION elecenergy(n)
  ...
  DO j=1,atoms1
     DO k=1,atoms2
        x=xyzmon1(n,j,1)-xyzmon2(n,k,1)
        y=xyzmon1(n,j,2)-xyzmon2(n,k,2)
        z=xyzmon1(n,j,3)-xyzmon2(n,k,3)
        dist=(x**2+y**2+z**2)**.5
...
function Inducenergy(n)
  ...
  real*8::Inducenergy
  integer,intent(in)::n
  real*8::dist,sum1,x,y,z
  integer::i,j,atoms1,atoms2
  real*8,dimension(3)::xvec,store,xyzdisp
...
!!!!!!!!!!!!!!!!!!! charge penetration contribution
  DO i=1,atoms1
     DO j=1,atoms2
        x=xyzmon1(n,i,1)-xyzmon2(n,j,1)
        y=xyzmon1(n,i,2)-xyzmon2(n,j,2)
        z=xyzmon1(n,i,3)-xyzmon2(n,j,3)
        dist=(x**2+y**2+z**2)**.5
...
!!!!!!!!!!!!!!!!!!!!!!!!!!spring energy 
     ...
     xvec(1)=x;xvec(2)=y;xvec(3)=z
     sum1=sum1+.5*springcon1*dot_product(xvec,xvec)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!interaction energy between all induction charges on monomer1
     do j=1,atoms1
        if(i.ne.j) then
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!atom-atom
           x=(xyzmon1(n,i,1)-xyzmon1(n,j,1))
           y=(xyzmon1(n,i,2)-xyzmon1(n,j,2))
           z=(xyzmon1(n,i,3)-xyzmon1(n,j,3))
           dist=((x)**2+(y)**2+(z)**2)**.5
           sum1=sum1+.5*screen(i,j,1,1,dist)*(-shellcharge1(i))*(-shellcharge1(j))/(dist)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!shell-shell
           x=(nxyzshell1(n,i,1)-nxyzshell1(n,j,1))
           y=(nxyzshell1(n,i,2)-nxyzshell1(n,j,2))
           z=(nxyzshell1(n,i,3)-nxyzshell1(n,j,3))
           dist=((x)**2+(y)**2+(z)**2)**.5
           sum1=sum1+.5*screen(i,j,1,1,dist)*(shellcharge1(i))*(shellcharge1(j))/(dist)
!!!!!!!!!!!!!!!!!!!!!!!!!atom-shell
           x=(xyzmon1(n,i,1)-nxyzshell1(n,j,1))
           y=(xyzmon1(n,i,2)-nxyzshell1(n,j,2))
           z=(xyzmon1(n,i,3)-nxyzshell1(n,j,3))
           dist=((x)**2+(y)**2+(z)**2)**.5
...
!!!!!!!!!!!!!!!!!!!!!!!!!interaction energy between monomers
     do j=1,atoms2
        ...
        x=(nxyzshell1(n,i,1)-xyzmon2(n,j,1))
        y=(nxyzshell1(n,i,2)-xyzmon2(n,j,2))
        z=(nxyzshell1(n,i,3)-xyzmon2(n,j,3))
        dist=((x)**2+(y)**2+(z)**2)**.5
        sum1=sum1+screen(i,j,1,2,dist)*(shellcharge1(i))*(chargeatoms2(j))/(dist)
     enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!interaction energy between all induction charges on monomer2
     do j=1,atoms2
        if(i.ne.j) then
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!atom-atom
           x=(xyzmon2(n,i,1)-xyzmon2(n,j,1))
           y=(xyzmon2(n,i,2)-xyzmon2(n,j,2))
           z=(xyzmon2(n,i,3)-xyzmon2(n,j,3))
           dist=((x)**2+(y)**2+(z)**2)**.5
           sum1=sum1+.5*screen(i,j,2,2,dist)*(-shellcharge2(i))*(-shellcharge2(j))/(dist)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!shell-shell
           x=(nxyzshell2(n,i,1)-nxyzshell2(n,j,1))
           y=(nxyzshell2(n,i,2)-nxyzshell2(n,j,2))
           z=(nxyzshell2(n,i,3)-nxyzshell2(n,j,3))
           dist=((x)**2+(y)**2+(z)**2)**.5
           sum1=sum1+.5*screen(i,j,2,2,dist)*(shellcharge2(i))*(shellcharge2(j))/(dist)
!!!!!!!!!!!!!!!!!!!!!!!!!atom-shell
           x=(xyzmon2(n,i,1)-nxyzshell2(n,j,1))
           y=(xyzmon2(n,i,2)-nxyzshell2(n,j,2))
           z=(xyzmon2(n,i,3)-nxyzshell2(n,j,3))
           dist=((x)**2+(y)**2+(z)**2)**.5
...
!!!!!!!!!!!!!!!!!!!!!!!!!interaction energy between monomers
     do j=1,atoms1
        ...
        x=(xyzmon2(n,i,1)-xyzmon1(n,j,1))
        y=(xyzmon2(n,i,2)-xyzmon1(n,j,2))
        z=(xyzmon2(n,i,3)-xyzmon1(n,j,3))
        dist=((x)**2+(y)**2+(z)**2)**.5
        sum1=sum1+screen(i,j,2,1,dist)*(-shellcharge2(i))*(chargeatoms1(j))/(dist)

        x=(nxyzshell2(n,i,1)-xyzmon1(n,j,1))
        y=(nxyzshell2(n,i,2)-xyzmon1(n,j,2))
        z=(nxyzshell2(n,i,3)-xyzmon1(n,j,3))
        dist=((x)**2+(y)**2+(z)**2)**.5
        sum1=sum1+screen(i,j,2,1,dist)*(shellcharge2(i))*(chargeatoms1(j))/(dist)
     enddo
  enddo         !!!!!!!!!!!!!!!!!end loop over induction sites i on monomer 1
...
function dhfenergy(n)
  ...
  integer,intent(in)::n
  INTEGER::atoms1,atoms2,i,j,k
  REAL*8::sum,dist,x,y,z,h_energy
...
function dexchenergy(atom1,atom2,n)
  ...
  if ( ( abs(Exchatom2(atom2)) < small_coeff ) .or. ( abs(Exchatom1(atom1)) < small_coeff ) ) then
     ...
     x=xyzmon1(n,atom1,1)-xyzmon2(n,atom2,1)
     y=xyzmon1(n,atom1,2)-xyzmon2(n,atom2,2)
     z=xyzmon1(n,atom1,3)-xyzmon2(n,atom2,3)
     dist=(x**2+y**2+z**2)**.5
